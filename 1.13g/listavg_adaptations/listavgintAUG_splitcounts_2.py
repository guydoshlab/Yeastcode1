...
def totalquant(counts_filestring,GFFgen_filename,utrgfffilename,utr5gfffilename,bp5_0,bp3_0,ignoreutr5,ignoreutr3,shift,normalizepause,eliminate,start_offset,end_offset,startpauseoffset,termpauseoffset,termstackpauseoffset,pausehalfregion,covlength,future1,future2):
	...
	genedictionary_intAUG["headers"]=["1stinternalAUG_"+samplename]
	newdictionary["headers"]=["extendedUTR_"+samplename,"annotatedUTR_"+samplename,"extendedUTRRAW_"+samplename,"annotatedUTRRAW_"+samplename]
	newdictionary2["headers"]=["firstgenereads_"+samplename,"lastgenereads_"+samplename,"firstgenereadsRAW_"+samplename,"lastgenereadsRAW_"+samplename]
...
#			stopcodon=genesequence[end-3:end]	# In case you want the stop codon, here it is.
			...
			firstAUG=findAUG(genesequence, start, end)
			if firstAUG!=-1:
				genedictionary_intAUG[feature.id]=countintAUG(genecounts,startpauseoffset,start_special,end_special,normalizepause,pausehalfregion,end,start,firstAUG)
			else:
				genedictionary_intAUG[feature.id]=[-10]
			
			splitdistance = 100	# Distance used for computing the beginning and end of main ORF (in nt). This could be brought to front panel for easier changes.
			newdictionary[feature.id]=splitUTR5count(genecounts,end,start,bp5_0,rawconvert)	# Compute reads in annotated and extended 5'UTR.
			newdictionary2[feature.id]=splitORFcount(genecounts,end,start,splitdistance,rawconvert)	# Compute reads in beginning/end of main ORF.
	...
	return [genedictionarygeneral,genedictionary1,genedictionary2,genedictionary_intAUG,newdictionary,newdictionary2]		# You can add genedictionary3 for reporting other kinds of information if used above.
...
def findAUG(genesequence,start,end):
	for position in range(start+3,end-3):		# Loop through 1 base at a time and get ATGs in all 3 frames. Stop before running off end.
		if genesequence[position:position+3]=="ATG":
			return position
	return -1

def countintAUG(genecounts,startpauseoffset,start_special,end_special,normalizepause,pausehalfregion,end,start,firstAUG):
	lengc=len(genecounts)
		
	# Define the start codon pause score, doing a check to make sure you're not running off the end of the list.
	if ((firstAUG-startpauseoffset)-pausehalfregion<0 or (firstAUG-startpauseoffset)+pausehalfregion+1>lengc):
		startpausescore=-1
	else:
		startpausescore=sum(genecounts[(firstAUG-startpauseoffset)-pausehalfregion:(firstAUG-startpauseoffset)+pausehalfregion+1])/(2*pausehalfregion+1)
	
	# Normalize the pause scores.
	if normalizepause==0:
		ORFaverage=1		# New can set a denom of 1 if filtermodule is -1.
	else:
		ORFaverage=sum(genecounts[start_special:end_special])/len(genecounts[start_special:end_special])	
	...
	return [startpausescore]

# Function that counts up reads in extended and annotated UTR regions separately.
def splitUTR5count(genecounts,end,start,bp5_0,rawconvert):
	if bp5_0==0:
		print "bp_5 should not be 0."
		exit()
		
	extendedUTR=sum(genecounts[0:start])/(start/float(1000))
	annotatedUTR=sum(genecounts[bp5_0:start])/((start-bp5_0)/float(1000))
	extendedUTRRAW=int(round(sum(genecounts[0:start])/rawconvert))
	annotatedUTRRAW=int(round(sum(genecounts[bp5_0:start])/rawconvert))
	
	return [extendedUTR,annotatedUTR,extendedUTRRAW,annotatedUTRRAW]

# Function that counts up reads from 0 - split distance and end - split distance of main ORF.
def	splitORFcount(genecounts,end,start,splitdistance,rawconvert):
	firstgenereads=sum(genecounts[start:start+splitdistance])/(splitdistance/float(1000))
	lastgenereads=sum(genecounts[end-splitdistance:end])/(splitdistance/float(1000))
	firstgenereadsRAW=int(round(sum(genecounts[start:start+splitdistance])/rawconvert))
	lastgenereadsRAW=int(round(sum(genecounts[end-splitdistance:end])/rawconvert))

	return [firstgenereads,lastgenereads,firstgenereadsRAW,lastgenereadsRAW]
