...
def totalquant(counts_filestring,GFFgen_filename,utrgfffilename,utr5gfffilename,bp5_0,bp3_0,ignoreutr5,ignoreutr3,shift,normalizepause,eliminate,start_offset,end_offset,startpauseoffset,termpauseoffset,termstackpauseoffset,pausehalfregion,covlength,future1,future2):
	...
	genedictionary_introns["headers"]=["totalintronreads_"+samplename,"totalintronreadsRAW_"+samplename]
...
#			stopcodon=genesequence[end-3:end]	# In case you want the stop codon, here it is.
			...
			genedictionary_introns[feature.id]=getintroncounts(introncounts,intronsequence,rawconvert)
	...
	return [genedictionarygeneral,genedictionary1,genedictionary2,genedictionary_introns]		# You can add genedictionary3 for reporting other kinds of information if used above.
...
def getintroncounts(introncounts,intronsequence,rawconvert):
    if introncounts!=-1 and intronsequence!=-1:
        intronlength=len(introncounts)
    	totalintronreads=float(1000)*sum(introncounts)/intronlength
        totalintronreadsRAW=int(round(sum(introncounts)/rawconvert))
    else:
        totalintronreads=-10
        totalintronreadsRAW=-10
       
    return [totalintronreads,totalintronreadsRAW]
